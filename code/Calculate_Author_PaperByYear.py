...
result_author_work_year=dict()
end_year=2024

for author_id,work_list in tqdm(author_work_dict.items()):
    if author_id not in result_author_work_year:
        result_author_work_year[author_id]=dict([(i,0) for i in range(1980,end_year+1)])
    ...
    for work_id in work_list:
        ...
        result_author_work_year[author_id][work_year]+=1

with open(os.path.join('..','result_alltime','Dict_Author_PaperByYear'),'wb') as f:
    pickle.dump(result_author_work_year,f)